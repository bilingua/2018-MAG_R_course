...
```{r 1.0}
shva <- read.csv('C:/Documents/Onik/R/duryagin_ReductionRussian.txt', sep = '\t')
...
```{r 1.2}
# f1 boxplot
ggplot(shva, aes(vowel, f1))+
  geom_boxplot(aes(fill = vowel))+
  coord_flip()
# f2 boxplot
ggplot(shva, aes(vowel, f2))+
  geom_boxplot(aes(fill = vowel))+
  coord_flip()
```
...
```{r 1.3}
boxplot(shva$f1[shva$vowel == 'a'])$out
```
...
# 1.7 
```{r
mix_model <- lmer(f1 ~ f2 + (1|vowel), shva)
#1.7.1
```
...
#1.7.4
```{r
shva$mix_pred <- predict(mix_model)
...
```{r 2.0}
elp <- read.csv('C:/Documents/Onik/R/ELP.csv', sep = ',')
```
...
```{r
mix_model2 <- lmer(Mean_RT ~ log(SUBTLWF) + (1|POS), elp)
```
...
```{r 2.4.3}
elp$mix_pred <- predict(mix_model2)
ggplot(elp, aes(log(SUBTLWF), Mean_RT))+
